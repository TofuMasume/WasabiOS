...
use core::{
    arch::asm, cmp::min, fmt, fmt::Write, mem::offset_of, mem::size_of, panic::PanicInfo,
    ptr::null_mut, writeln,
};
...
fn efi_main(_image_handle: EfiHandle, efi_system_table: &EfiSystemTable) {
    ...
    draw_str_fg(&mut vram, 256, 256, 0xffffff, "Hello, Hanashi!");
...
const MEMORY_MAPPED_BUFFER_SIZE: usize = 0x8000;

struct MemoryMapHolder {
    memory_map_buffer: [u8; MEMORY_MAPPED_BUFFER_SIZE],
...
impl MemoryMapHolder {
    pub const fn new() -> MemoryMapHolder {
        MemoryMapHolder {
            memory_map_buffer: [0; MEMORY_MAPPED_BUFFER_SIZE],
            memory_map_size: MEMORY_MAPPED_BUFFER_SIZE,
            map_key: 0,
            descriptor_size: 0,
...
trait Bitmap {
    ...
    fn pixel_at_mut(&mut self, x: i64, y: i64) -> Option<&mut u32> {
        if self.is_in_x_range(x) && self.is_in_y_range(y) {
            // SAFETY: (x, y) is always validation by the checks above.
...
/// # Safety
///
/// (x,y) mut be a valid point in the buf.
